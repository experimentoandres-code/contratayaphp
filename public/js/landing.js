...
(function cintaInstalacion() {
  ...
  if (Instalar.disponible) { conBoton(); return; }

  // Salida por defecto: hay navegadores donde el aviso de "se puede
  // instalar" no llega nunca, y sin esto no aparecía absolutamente nada.
  pintar('Tenela en la pantalla de inicio y enterate al toque',
         '<a class="cinta-btn" href="#instalar">Ver cómo</a>');

  // Si el aviso llega después, la cambiamos por el botón de instalar.
...
(function whatsappComercios() {
  const btn = document.getElementById('btnZonaComercio');
  if (!btn || typeof WHATSAPP_COMERCIOS === 'undefined' || !WHATSAPP_COMERCIOS) return;
  btn.setAttribute('href', enlaceWhatsapp('Hola, quiero la zona de mi comercio en Contratá Ya.', WHATSAPP_COMERCIOS));
...
(function planes() {
  ...
  PLANES.forEach(p => {
    ...
    art.innerHTML = `
      ${p.destacado ? '<span class="plan-cinta">Plan Pro</span>' : ''}
      <h3>${p.nombre}</h3>
      <p class="plan-precio">${p.precioTexto}</p>
      <p class="plan-resumen">${p.resumen}</p>
      <ul class="plan-incluye">${incluye}${excluye}</ul>
      ${p.id === 'pro'
        ? `<a class="btn btn-plomo" href="${enlaceWhatsapp('Hola, quiero el plan Pro de Contratá Ya.')}" target="_blank" rel="noopener noreferrer">Pedir plan</a>`
        : '<p class="plan-nota">Siempre gratis · se activa por WhatsApp</p>'}`;
...
/* ── Cifra real de profesionales ─────────────────────────── */
(function cifraProfesionales() {
  ...
  const cargar = async () => {
    try {
      ...
      if (n == null || n === '') return;
      el.textContent = Number(n).toLocaleString('es-AR');
    } catch (e) { console.warn('[cifra]', e); }
  };
...
document.addEventListener('click', (e) => {
  const tut = e.target.closest && e.target.closest('.btn-tutorial-guia, #btnTutorial');
  if (tut) {
    e.preventDefault();
    if (typeof verTutorial === 'function') verTutorial();
  }
});

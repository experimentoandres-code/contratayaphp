...
const RUBROS = [
  ...
  { id: 'fumigacion',    nombre: 'Fumigación',       glifo: '※' },
  { id: 'jardineria',    nombre: 'Parquización',     glifo: '❦' },
  { id: 'mantenimiento', nombre: 'Mantenimiento',    glifo: '⊕' },
  { id: 'casero',        nombre: 'Casero (cuidado de propiedades)', glifo: '⌂' },
  { id: 'limpieza',      nombre: 'Limpieza de casas / Hoteles', glifo: '◇' },
  { id: 'fletes',        nombre: 'Fletes y mudanzas', glifo: '▸' },
  { id: 'electronica',   nombre: 'Técnico electrónico', glifo: '◎' },
  { id: 'cerrajero',     nombre: 'Cerrajero',         glifo: '⬧' },
...
// WhatsApp de Contratá Ya. Formato internacional sin +: 549XXXXXXXXXX
// El mismo número atiende comercios (landing) y verificación de cuentas (perfil).
const WHATSAPP_CONTRATA = '5492246552086';
const WHATSAPP_COMERCIOS = WHATSAPP_CONTRATA;
const INSTAGRAM_CONTRATA = 'https://www.instagram.com/contrataya.pro/';
const FACEBOOK_CONTRATA = 'https://www.facebook.com/people/Contrata-ya-pro/61593293355517/';

function enlaceWhatsapp(mensaje, numero) {
  const dest = numero || WHATSAPP_CONTRATA;
  const texto = encodeURIComponent(mensaje);
  return dest
    ? `https://wa.me/${dest}?text=${texto}`
...
const PROFESIONALES = [
  ...
  {
    id: 16, nombre: 'Romina Paredes', rubro: 'limpieza', localidad: 'San Bernardo',
    foto: '/img/gente/c06.svg',
    anios: 8, trabajos: 164, puntaje: 4.8, respuesta: 6,
    verificacion: ['telefono','email','cuit','identidad','zona'], plan: 'verificado',
    desde: 22000, bio: 'Limpieza de casas, departamentos de temporada y hoteles chicos. Entrega y recepción de alquileres, cambio de ropa blanca y productos propios.',
    especialidades: ['Casas de temporada', 'Hoteles', 'Check-in / check-out', 'Ropa blanca'],
    resenas: [
      { autor: 'Paula N.', local: 'Costa Chica', puntaje: 5, texto: 'Deja la casa lista para recibir huéspedes. Nunca un reclamo.' },
      { autor: 'Osvaldo M.', local: 'Mar de Ajó', puntaje: 5, texto: 'Atiende tres unidades mías. Puntual y no hay que explicarle dos veces.' }
    ]
  }
];
...
const PLANES = [
  {
    id: 'gratis', nombre: 'Gratis Verificado', precio: 0, precioTexto: 'Sin costo',
    resumen: 'Para trabajar en la costa con cuenta activa y sello de verificado.',
    incluye: [
      'Cuenta activada y verificada por WhatsApp',
      'Perfil con rubro, zona y calificaciones',
      'Hasta en 3 localidades al mismo tiempo',
      'Contactos y matches sin límite',
      'Beneficios en comercios adheridos'
    ],
    excluye: [
      'Galería de fotos de tus trabajos en la ficha del mazo',
      'Link de tus redes sociales en tu ficha del mazo',
      'Primero en el mazo de tu localidad',
      'Publicarte en todas las localidades que necesites al mismo tiempo',
      'Aviso al toque cuando entra un pedido de tu oficio',
      'Presupuestador en el match (mano de obra por escrito)'
    ]
  },
  {
    id: 'pro', nombre: 'Pro', precio: 15000, precioTexto: '$15.000 por mes',
    resumen: 'Salís primero, cubrís más zona y te avisamos apenas entra un pedido.',
    destacado: true,
    incluye: [
      'Todo lo del plan Gratis Verificado',
      'Galería de fotos de tus trabajos en la ficha del mazo',
      'Publicarte en todas las localidades que necesites al mismo tiempo',
      'Link de tus redes sociales en tu ficha del mazo',
      'Primero en el mazo de tu localidad',
      'Aviso al toque cuando entra un pedido de tu oficio',
      'Presupuestador en el match: se lo mandás al cliente por escrito, solo mano de obra'
    ],
    excluye: []
  }
];
...
if (typeof module !== 'undefined') {
  module.exports = { LOCALIDADES, RUBROS, URGENCIAS, CAPAS_VERIFICACION, WHATSAPP_COMERCIOS, WHATSAPP_CONTRATA, INSTAGRAM_CONTRATA, FACEBOOK_CONTRATA, PROFESIONALES, CLIENTES, CRITERIOS, SPONSORS, PLANES };
}

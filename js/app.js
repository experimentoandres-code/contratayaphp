...
function registroGoogle() {
  abrirHoja(`
    <div class="verif-paso">
      ...
      <p style="color:var(--cal-2)">Verificando tu cuenta…</p>
    </div>`);
  setTimeout(() => {
    cerrarHoja();
    crearCuenta({ nombre: 'Andrés Gómez', correo: 'andres.gomez@gmail.com', metodo: 'Google' });
  }, 1700);
}
...
function registroCorreo() {
  ...
  $('#regEnviar').addEventListener('click', () => {
...
function pedirCodigo(nombre, correo) {
  abrirHoja(`
    ...
    <p>Mandamos un código a <b style="color:var(--cal)">${correo}</b>. En la demostración sirve cualquier número de seis cifras.</p>
  ...
  $('#regConfirmar').addEventListener('click', () => {
